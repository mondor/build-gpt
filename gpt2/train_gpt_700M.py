...
# torchrun --standalone --nproc_per_node=8 train_gpt_700M.py
...
class GPTConfig:
    ...
    n_layer: int = 24
    n_head: int = 12  # 1536/12 head_dim=128
    n_embed: int = 1536
...
class GPT(nn.Module):
    ...
    def configure_optimizers(self, weight_decay, learning_rate, device_type):
        ...
        optimizer = torch.optim.AdamW(optim_groups, lr=learning_rate, betas=(0.8, 0.95), eps=1e-10, fused=use_fused)
...
if __name__ == '__main__':
    ...
    total_batch_size = 524288  # 2**19, ~0.5M tokens (nanochat predicted ~same for d24)
    B = 16  # micro batch size (lowered from 32 since T doubled to 2048)
    T = 2048
    use_compile = True
    ...
    # nanochat-aligned schedule: trapezoidal (linear warmup -> constant -> linear warmdown)
    max_lr = 3e-4  # lowered from 6e-4 for the larger model under AdamW
    min_lr_frac = 0.05  # final LR as fraction of max_lr
    warmup_steps = 40  # nanochat uses 40 (vs GPT-2 paper's 715)
    warmdown_ratio = 0.65  # fraction of training dedicated to warmdown
    max_steps = 10700  # ~8:1 tokens:params ratio (nanochat target-param-data-ratio=8)
    ...
    def get_lr(it):
        ...
        # after training horizon, return min lr
        if it > max_steps:
        ...
        if it <= max_steps - warmdown_iters:
            return max_lr
        # linear warmdown from max_lr to max_lr * min_lr_frac
    ...
    # nanochat weight_decay=0.28 scaled by D_ref/D ≈ 1/8 for d24 -> ~0.035
    optimizer = raw_model.configure_optimizers(weight_decay=0.035, learning_rate=max_lr, device_type=device_type)
